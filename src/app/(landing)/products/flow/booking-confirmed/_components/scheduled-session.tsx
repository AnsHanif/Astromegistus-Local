...
const ScheduledSession = () => {
  const {
    ...
    category: PURCHASED_PRODUCTS_TYPE.SCHEDULED,
    limit: 5,
  });
  ...
  return (
    <div className="mt-4">
      <h2 className="text-size-large md:text-size-heading font-medium mb-2">
        Scheduled Live Sessions <span className="text-sm">({totalCount})</span>
      </h2>
          ...
          {allProducts.length > 0 ? (
            allProducts.map((product: any) => (
              <ReadingSessionCard
                ...
                classNames="mb-6"
                href={`/products/flow/manual-reading?productId=${product.productId}&itemId=${product.id}`}
              />
            ))
          ) : (
            <div className="text-center py-8">
              <p className="text-gray-500">
                No scheduled sessions available at the moment.
              </p>
            </div>
          )}
