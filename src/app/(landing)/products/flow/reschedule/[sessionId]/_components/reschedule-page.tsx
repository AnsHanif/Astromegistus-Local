...
import { useSessionPreparation, useTimeSlotsByDateAndId } from '@/hooks/query/booking-queries';
...
// Timezone constants
const TIME_ZONES = [
  { label: 'Pacific Time (UTC -8)', value: 'Pacific Time (UTC -8)' },
  { label: 'Mountain Time (UTC -7)', value: 'Mountain Time (UTC -7)' },
  { label: 'Central Time (UTC -6)', value: 'Central Time (UTC -6)' },
  { label: 'Eastern Time (UTC -5)', value: 'Eastern Time (UTC -5)' },
  { label: 'UTC', value: 'UTC' },
  { label: 'British Summer Time (UTC +1)', value: 'British Summer Time (UTC +1)' },
  { label: 'Central European Time (UTC +2)', value: 'Central European Time (UTC +2)' },
  { label: 'India Standard Time (UTC +5:30)', value: 'India Standard Time (UTC +5:30)' }
];
...
const ReschedulePage = ({ sessionId }: ReschedulePageProps) => {
  ...
  // Fetch session data
  const { data: sessionData, isLoading, error } = useSessionPreparation(sessionId);
  
  // Reschedule mutation
  const rescheduleBookingMutation = useRescheduleBooking();

  // Get provider ID from session data
  const providerId = sessionData?.data?.providerId;
  
  // Format selected date for API call (YYYY-MM-DD)
  const formattedSelectedDate = value && !Array.isArray(value) ? 
    moment(value).format('YYYY-MM-DD') : undefined;

  // Fetch time slots for the selected date and provider
  const { 
    data: timeSlotsData, 
    isLoading: isLoadingTimeSlots, 
    error: timeSlotsError 
  } = useTimeSlotsByDateAndId(
    providerId,
  ...
  useEffect(() => {
    if (timeSlotsError) {
      enqueueSnackbar('Failed to load available time slots', { variant: 'error' });
    }
  }, [timeSlotsError]);
  
  // Handle reschedule confirmation
  const handleConfirmReschedule = async () => {
    if (!selectedTime || !value || Array.isArray(value)) {
    ...
    try {
      ...
      enqueueSnackbar('Session rescheduled successfully!', { variant: 'success' });
      router.push('/dashboard/upcoming-sessions');
    } catch (error) {
  ...
  const currentDate = session?.selectedDate ? new Date(session.selectedDate).toLocaleDateString('en-US', {
    weekday: 'short',
    month: 'short', 
    day: 'numeric',
    year: 'numeric'
  ...
  // Get available time slots from API data
  const availableTimeSlots = timeSlotsData?.data?.filter(slot => slot.isAvailable) || [];
    ...
    <ProductInfoHeader title="Rescheduling">
      <div className="flex flex-col gap-6 md:gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 w-full">
            ...
            <div>
              <h2 className="text-lg font-semibold mb-4">
                ...
                {value && !Array.isArray(value) ? moment(value).format('MMMM Do, YYYY') : ''}
              </h2>
        ...
        <RescheduleCard
          ...
          avatar="/astrologist-2.jpg"
          sessionType={session?.session?.title || 'Session Type'}
          date={value && !Array.isArray(value) ? value.toLocaleDateString('en-US', {
            weekday: 'short',
            month: 'short', 
            day: 'numeric',
            year: 'numeric'
          }) : 'Select date'}
          time={selectedTime ? `${selectedTime} (${session?.session?.duration || 'Duration not set'})` : 'Select time'}
        />

        <Button
          onClick={() => setIsConfirmReschedule(true)}
          disabled={!selectedTime || !value || Array.isArray(value)}
      ...
      <ConfirmResheduleModal
        ...
        newDate={value && !Array.isArray(value) ? value.toLocaleDateString('en-US', {
          weekday: 'short',
          month: 'short', 
          day: 'numeric',
          year: 'numeric'
        }) : 'New date'}
        newTime={selectedTime || 'New time'}
