...
export const adminAPI = {
  ...
  getReadingOrders: (params?: ReadingOrderQueryParams) =>
    adminAxiosInstance.get<ReadingOrdersResponse>('/admin/orders/readings', { params }),
  getSessionOrders: (params?: SessionOrderQueryParams) =>
    adminAxiosInstance.get<SessionOrdersResponse>('/admin/orders/sessions', { params }),
  getReadingOrderById: (id: string) =>
    adminAxiosInstance.get<ReadingOrderResponse>(`/admin/orders/readings/${id}`),
  getSessionOrderById: (id: string) =>
    adminAxiosInstance.get<SessionOrderResponse>(`/admin/orders/sessions/${id}`),
  updateReadingOrderStatus: (id: string, data: UpdateOrderStatusRequest) =>
    adminAxiosInstance.patch<ReadingOrderResponse>(`/admin/orders/readings/${id}/status`, data),
  updateSessionOrderStatus: (id: string, data: UpdateOrderStatusRequest) =>
    adminAxiosInstance.patch<SessionOrderResponse>(`/admin/orders/sessions/${id}/status`, data),

  // S3 File Access
  getPresignedUrl: (key: string) =>
    adminAxiosInstance.get<{ message: string; status_code: number; data: { url: string; key: string; expiresIn: number } }>(`/s3/presigned-url?key=${encodeURIComponent(key)}`),
};
